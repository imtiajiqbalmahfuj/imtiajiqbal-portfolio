...
function mountHero(){
  ...
  track.innerHTML = window.SITE.tickerIcons.map(it=> `<span class="inline-flex items-center gap-2 mr-8 text-sm text-slate-600">
    <i data-lucide="${it.icon}"></i>${it.name}
  </span>`).join('')

  // Down button to Featured Projects
...
function mountSlideshow(){
  ...
  const slides = (window.SITE.projects || []).slice().reverse()
  slidesWrap.style.width = `${slides.length * 100}%`
  slidesWrap.innerHTML = slides.map(s=> `
    <div class="relative w-full shrink-0">
      <a href="projects.html#${s.id}" class="block">
        <img src="${s.image}" alt="${s.title}" class="w-full h-64 md:h-96 object-cover">
        <!-- Title removed for slideshow only -->
      </a>
    </div>
  `).join('')
  ...
  function go(i){
    ...
    slidesWrap.style.transform = `translateX(-${idx*(100/slides.length)}%)`
  }
...
function mountProjectsCarousel(){
  ...
  if(!wrap) return
  const items = window.SITE.projects
    ...
    $('#projectTrack').innerHTML = filtered.map(p=> `
      <div class="w-full sm:w-1/2 md:w-1/3 p-2">
        <div class="card h-full bg-white border border-slate-200 rounded-xl overflow-hidden flex flex-col">
          <a href="projects.html#${p.id}" class="block"><img src="${p.image}" class="w-full h-44 object-cover" alt="${p.title}"></a>
          <div class="p-4 flex flex-col gap-3 grow">
            ...
            <div class="mt-auto flex gap-2 justify-end">
              <a class="px-3 py-1.5 bg-white border rounded-xl hover:bg-black hover:text-white hover-smart" href="${p.github}" target="_blank"><i data-lucide="github"></i></a>
              <a class="px-3 py-1.5 bg-white border rounded-xl hover:bg-black hover:text-white hover-smart" href="projects.html#${p.id}" ><i data-lucide="external-link"></i><span class="sr-only">See more</span></a>
            </div>
          </div>
        </div>
      </div>
...
function mountAchievementsPreview(){
        ...
        <div class="grid gap-2">
          ${list.map(a=>`
            <div class="flex items-start justify-between gap-3">
              ...
              <a class="px-3 py-1.5 bg-white border rounded-xl hover:bg-black hover:text-white hover-smart" href="${a.link}" target="_blank"><i data-lucide="external-link"></i></a>
            </div>
          `).join('')}
        </div>
      </div>
  ...
  wrap.innerHTML = [
    card(A.fellowships,"Fellowships, Awards & Research Grants"),
    card(A.licenses,"Licenses & Certifications"),
    card(A.workshops,"Workshops & Presentations"),
    card(A.volunteering,"Volunteering")
  ].join('')
}
